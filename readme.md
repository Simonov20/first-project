...
git branch -a - проспотр всех веток локально так и на удаленном репозитории      
...
flowchart TD;
    ...
    v3("git add . 
    (добавление файлов в для коммита)") -->
    v4("git commit -m ['сообщение']") -->
    v5("git push -u origin [название_ветки]
    (привязка ветки к GitHub репозиторию)") -->
    v6("pull request
    (не обязательно, но рекомендуемо для ревью и согласования;
    в GitHub или по ссылке, которую дает Git после push)") -->
    ...
    v9("git push
    (чтобы изменения накатились в master ветку)");
```
 
git pull - забрать изменения из удалённого репозитория аналогично git merge + push из схемы выше   
