...
```mermaid
flowchart TD;
git branch [название_ветки] --> git switch [название_ветки] --> git add . --> git commit -m ['сообщение'] --> git push -u origin [название_ветки] --> pull request(не обязательно, но рекомендуемо для ревью и согласования) --> git switch master(та ветка, с которой надо объединиться) --> git merge [название_ветки](которую надо объединить, в нашем случае с веткой master)  
```
